...
- Modül bitince status `Done` yapılır, `PROGRESS.md` ve `.agent/WORKLOG.md` güncellenir.
...
| 11-03 | Backlog | Trigonometrik Kök Avcısı | `MAT.11.1.2.x` | Dalga üzerinde kök işaretleyici ve faz tarayıcı | Denklem köklerini grafikle eşleştirir. |
| 11-04 | Ready | Üstel Büyüme Reaktörü | `MAT.11.1.3.x` | Taban sürgüsüyle artan/azalan büyüme sahnesi | Büyüme/azalma farkı tek manipülasyonla görünür. |
| 11-05 | Ready | Logaritma Ters Ayna Odası | `MAT.11.1.4.x`, `MAT.11.1.5.x` | Üstel grafiği `y=x` aynasında logaritmaya çeviren düzenek | Logaritmayı ters işlem olarak sezdirir. |
| 11-06 | Backlog | Richter-Desibel Ölçek Simülatörü | `MAT.11.1.6.x` | Ses/deprem verisini log ölçeğinde sıkıştıran ölçer | Gerçek yaşam bağlantısı güçlüdür. |
| 11-07 | Backlog | Fonksiyon Bileşke Portları | `MAT.11.1.7.x` | `g` makinesinin çıktısını `f` makinesine takan port sistemi | Bileşkeyi kablo ve makine zinciriyle gösterir. |
...
| 11-09 | Ready | Dörtgen Ayrıştırma Masası | `MAT.11.2.1.x` | Dörtgeni köşegeninden iki üçgene ayıran kesme masası | 360° ve alan mantığını önceki üçgen deneylerine bağlar. |
...
| 11-14 | Ready | Korelasyon Serpilme Radarı | `MAT.11.3.1.x` | Nokta bulutu, eğilim çizgisi ve pozitif/negatif tarayıcı | Veri ilişkiselliğini güçlü görsel pattern olarak gösterir. |
| 11-15 | Backlog | Medya Korelasyon Denetçisi | `MAT.11.3.2.x` | Dış veri iddiasını tutarlılık testinden geçiren analiz ekranı | Eleştirel veri okuryazarlığı kazandırır. |
