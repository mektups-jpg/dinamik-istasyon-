...
interface ConditionalFilterControlsProps {
  mission: MissionStep;
  activeIndex: number;
...
const ruleCopy = [
  'Koşul gerçekleştiğinde eski altı seçenekli evren artık kullanılmaz.',
  'Hedef olay, sadece koşuldan sonra kalan yeni evrende aranır.',
  'Pay uygun sonuç sayısıdır; payda koşuldan sonra kalan sonuç sayısıdır.',
];

export function ConditionalFilterControls({
  mission,
  activeIndex,
  target,
  measure,
  ...
  onCheck,
  onReset,
}: ConditionalFilterControlsProps) {
  return (
    <aside className="min-w-0 rounded-[30px] border border-white/14 bg-black/38 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.26)] backdrop-blur-2xl xl:p-5">
      <p className="font-mono text-[10px] font-black uppercase tracking-[0.28em] text-sky-100/55">koşul {activeIndex + 1}/3</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <Metric label="evren" value={measure.universe} />
        <Metric label="uygun" value={measure.favorable} />
        <Metric label="kesir" value={measure.selectedFractionLabel} />
        <Metric label="ray" value={measure.targetLabel} />
      </div>

      <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.045] p-3">
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">koşullu kural</p>
        <p className="mt-2 text-xs leading-relaxed text-white/70">{ruleCopy[activeIndex]}</p>
      </div>

      <div className="mt-4 grid gap-2">
        <MiniChoice label="çift geldi filtresi" active={state.conditionOn} expected={target.id === 'condition'} onClick={onToggleCondition} />
        <MiniChoice label="4'ten büyük filtresi" active={state.targetOn} expected={target.id === 'target'} onClick={onToggleTarget} />
        <MiniChoice label="1/3 tokenı" active={state.selectedFraction === '1/3'} expected={target.id === 'fraction'} onClick={onSelectFraction} />
        <MiniChoice label="rapor mührü" active={state.reportSealed} expected={target.id === 'fraction'} onClick={onToggleSeal} seal />
      </div>

      <div className="mt-4 grid gap-3">
        <SciFiButton data-testid="conditional-filter-check" onClick={onCheck} className="min-h-[50px]" icon={<Check className="h-4 w-4" />}>
          Filtreyi Onayla
        </SciFiButton>
        <SciFiButton data-testid="conditional-filter-reset" variant="secondary" onClick={onReset} className="min-h-[50px]" icon={<RotateCcw className="h-4 w-4" />}>
          Bu Görevi Sıfırla
        </SciFiButton>
      </div>
    </aside>
