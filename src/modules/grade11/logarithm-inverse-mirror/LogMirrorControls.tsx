...
import { LogMirrorMeasure, LogMirrorTarget } from './types';
import { formatBase, formatPercent, formatValue } from './logMirrorModel';

interface LogMirrorControlsProps {
  mission: MissionStep;
  activeIndex: number;
...
export function LogMirrorControls({ mission, activeIndex, target, measure, missionOk, onCheck, onReset }: LogMirrorControlsProps) {
  const successCopy = target.kind === 'inverse' ? 'Ayna hedefe kilitlendi; üstel eğri logaritmaya dönüştü.' : 'Ayna ve taban hedef davranışa kilitlendi.';
  const ruleCopy = target.kind === 'inverse'
    ? (measure.mirrorPower < 0.94 ? 'Ters fonksiyon için x ve y rolleri değişmeli.' : 'x ve y rolleri değişti: üstel eğri logaritma oldu.')
    : measure.mode === 'decay'
      ? '0<a<1 iken logaritma grafiği azalır.'
      : 'a>1 iken logaritma grafiği artar.';
  ...
  return (
    <aside className="min-w-0 rounded-[30px] border border-white/14 bg-black/38 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.26)] backdrop-blur-2xl xl:p-5">
      <p className="font-mono text-[10px] font-black uppercase tracking-[0.28em] text-cyan-100/55">ters ayna {activeIndex + 1}/3</p>
      ...
      <div className={`mt-4 rounded-2xl border p-3 xl:p-4 ${missionOk ? 'border-emerald-300/25 bg-emerald-300/10 text-emerald-100' : 'border-cyan-300/20 bg-cyan-300/[0.07] text-cyan-100'}`}>
        ...
        <p className="mt-1 text-xs leading-relaxed opacity-75">{missionOk ? successCopy : target.hint}</p>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-2">
        <Metric label="taban" value={`a=${formatBase(measure.base)}`} />
        <Metric label="ayna" value={formatPercent(measure.mirrorPower)} />
        <Metric label="mod" value={modeLabel[measure.mode]} />
        <Metric label="log_a(2)" value={formatValue(measure.sampleRight)} />
      </div>
      ...
      <div className="mt-4 grid gap-3">
        <SciFiButton data-testid="logarithm-mirror-check" onClick={onCheck} className="min-h-[50px]" icon={<Check className="h-4 w-4" />}>
          Aynayı Onayla
        </SciFiButton>
        <SciFiButton data-testid="logarithm-mirror-reset" variant="secondary" onClick={onReset} className="min-h-[50px]" icon={<RotateCcw className="h-4 w-4" />}>
          Sıfırla
        </SciFiButton>
      </div>
      ...
      <div className="mt-5 hidden rounded-2xl border border-white/10 bg-white/[0.045] p-4 xl:block">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-white/58">Logaritma, üstel makinenin aynada ters çevrilmiş halidir.</p>
          </div>
        </div>
      </div>
    </aside>
