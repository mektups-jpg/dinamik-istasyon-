...
import { MODULE_ID, OptimizationMission, OptimizationTool, toolCopy } from './optimizationTerrainModel';

interface OptimizationTerrainControlsProps {
  mission: OptimizationMission;
  missionIndex: number;
  missionCount: number;
  tool: OptimizationTool | null;
  solved: boolean;
  onToolChange: (tool: OptimizationTool) => void;
  onCheck: () => void;
  onNext: () => void;
}
...
export function OptimizationTerrainControls({
  ...
  onCheck,
  onNext,
}: OptimizationTerrainControlsProps) {
  const checkLabel = tool === null ? 'Önce Karar Seç' : 'Araziyi Test Et';
