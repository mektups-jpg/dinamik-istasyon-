...
interface Grade2MissionFrameProps {
  title: string;
  subtitle: string;
...
interface Grade2CompletionProps {
  title: string;
  message: string;
...
interface Grade2ChoiceButtonProps {
  children: React.ReactNode;
  onClick: () => void;
  disabled?: boolean;
  selected?: boolean;
...
export function Grade2MissionFrame({
  title,
  subtitle,
  icon,
  accent,
  ...
  return (
    <div className="relative h-full w-full overflow-x-hidden overflow-y-auto bg-[#070B12] pb-32 text-white">
      ...
      <div className="pointer-events-none fixed left-[7%] top-[7%] h-72 w-72 rounded-full bg-[#00E5FF]/10 blur-[110px]" />
...
export function Grade2Panel({ children }: { children: React.ReactNode }) {
  return (
...
export function Grade2ChoiceButton({ children, onClick, disabled, selected, status = 'idle', accent }: Grade2ChoiceButtonProps) {
  const stateClass = status === 'success'
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`min-h-16 rounded-3xl border px-4 py-3 text-lg font-black transition ${stateClass} disabled:cursor-not-allowed disabled:opacity-60`}
      style={selected ? { boxShadow: `0 0 26px ${glow}` } : undefined}
    >
      {children}
    </motion.button>
  );
...
export function Grade2Completion({ title, message, atoms, accent, onRestart }: Grade2CompletionProps) {
  return (
    <motion.section
