...
import { ArrowLeft, CheckCircle2, Home, PieChart, RotateCcw, Sparkles, Star } from 'lucide-react';
import { AstroBot, type BotMessage } from '../../../components/ui/AstroBot';
import { useAtomStore } from '../../../store/useAtomStore';
import { useGameStore } from '../../../store/useGameStore';
import {
  createFractionBalanceTasks,
  FRACTION_BALANCE_ATOMS,
  type FractionBalanceTask,
  type FractionShape,
} from './fractionBalance4Tasks';

const MODULE_ID = 'fraction-balance-4';

export default function FractionBalance4App() {
  const { unlockAtom, unlockModule } = useAtomStore();
  const { addScore } = useGameStore();
  const [tasks, setTasks] = useState<FractionBalanceTask[]>(() => createFractionBalanceTasks());
  ...
  useEffect(() => {
    ...
    FRACTION_BALANCE_ATOMS.forEach((atom) => unlockAtom(atom.id));
  ...
  const restart = () => {
    setTasks(createFractionBalanceTasks());
    setTaskIndex(0);
    setSelected(null);
    setFeedback('idle');
    setIsComplete(false);
  ...
  const botMessage: BotMessage = useMemo(() => {
    if (isComplete) {
      return { id: 900, text: 'Kesir terazisi dengede! Kesir türlerini, denk kesirleri, karşılaştırmayı ve eş paydalı işlemleri kurdun.', type: 'success' };
    }
    if (feedback === 'success') {
  ...
  return (
    <div className="relative h-full w-full overflow-x-hidden overflow-y-auto bg-[#070B12] pb-32 text-white">
      ...
      <div className="pointer-events-none fixed right-[9%] top-[18%] h-64 w-64 rounded-full bg-[#B388FF]/10 blur-[110px]" />
      <div className="pointer-events-none fixed bottom-[5%] left-[38%] h-80 w-80 rounded-full bg-[#34D399]/10 blur-[125px]" />
      ...
      <header className="relative z-20 mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-7">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-xl font-black leading-tight text-[#2EE7FF] md:text-2xl">Kesir Denge Terazisi</h1>
            <p className="mt-1 font-mono text-[10px] font-black uppercase tracking-[0.18em] text-white/48 md:text-xs">
              İlkokul 4. Sınıf / Denk Kesir, Karşılaştırma ve İşlem
            </p>
          </div>
        </div>
      ...
      <main className="relative z-10 mx-auto grid min-h-[calc(100vh-112px)] w-full max-w-7xl grid-cols-1 gap-5 px-4 py-3 md:px-7 lg:grid-cols-[minmax(0,1fr)_390px]">
        <section
          data-testid="fraction-balance-4-stage"
          ...
          <AnimatePresence mode="wait">
            {isComplete ? (
              ...
              >
                ...
                <FractionStage task={task} feedback={feedback} />
              </motion.div>
            )}
          </AnimatePresence>
        </section>
        ...
        <aside
          data-testid="fraction-balance-4-control-panel"
        ...
        >
          {isComplete ? (
            <div className="flex h-full min-h-[360px] flex-col items-center justify-center text-center">
              ...
              <h2 className="mt-5 text-2xl font-black">Terazi dengede!</h2>
              <p className="mt-2 max-w-xs text-sm font-bold leading-relaxed text-white/58">Kesir parçalarını doğru okudun.</p>
            </div>
          ) : (
            <>
              ...
              <div
                data-testid="fraction-balance-4-feedback"
...
function FractionStage({ task, feedback }: { task: FractionBalanceTask; feedback: 'idle' | 'success' | 'error' }) {
  return (
    <div className="rounded-[2rem] border border-white/10 bg-black/24 p-5">
      <div className="rounded-[2rem] border border-white/10 bg-[#081522]/92 p-5">
        <div className="mb-4 flex items-center justify-center gap-2 text-center">
          ...
          <p className="font-mono text-[10px] font-black uppercase tracking-[0.2em] text-white/48">Canlı kesir</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          {task.fractions.map((fraction, index) => (
            <FractionTank key={`${fraction.label}-${index}`} fraction={fraction} accent={task.accent} />
          ))}
        </div>
...
function FractionTank({ fraction, accent }: { fraction: FractionShape; accent: string }) {
  const filled = Math.min(fraction.numerator, fraction.denominator);
  const fractionHint = fraction.numerator > fraction.denominator
    ? '1 tamdan büyük'
  ...
  return (
    <div className="rounded-3xl border bg-white/[0.05] p-4" style={{ borderColor: `${accent}44` }}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <p className="text-3xl font-black text-white">{fraction.label}</p>
        <FractionBadge numerator={fraction.numerator} denominator={fraction.denominator} accent={accent} />
      </div>
      <div className="grid min-h-28 gap-2" style={{ gridTemplateColumns: `repeat(${fraction.denominator}, minmax(0, 1fr))` }}>
        {Array.from({ length: fraction.denominator }).map((_, index) => (
          <motion.div
            key={index}
            initial={{ scaleY: 0.55, opacity: 0.65 }}
            animate={{ scaleY: 1, opacity: 1 }}
            transition={{ delay: index * 0.04 }}
            className="origin-bottom rounded-2xl border"
            style={{
              borderColor: `${accent}55`,
              background: index < filled ? `linear-gradient(180deg, ${accent}, rgba(255,255,255,0.18))` : 'rgba(255,255,255,0.06)',
              boxShadow: index < filled ? `0 0 18px ${accent}44` : undefined,
            }}
          />
        ))}
      </div>
      <div className="mt-3 rounded-2xl border px-3 py-2 text-center text-xs font-black text-white/76" style={{ borderColor: `${accent}44`, backgroundColor: `${accent}12` }}>
        {fractionHint}
      </div>
    </div>
  );
...
function FractionBadge({ numerator, denominator, accent }: { numerator: number; denominator: number; accent: string }) {
  return (
    <div className="flex min-w-16 flex-col items-center rounded-2xl border bg-black/24 px-3 py-2 text-center font-black" style={{ borderColor: `${accent}55` }}>
      <span className="text-xl leading-none" style={{ color: accent }}>{numerator}</span>
      <span className="my-1 h-[2px] w-9 rounded-full bg-white/45" />
      <span className="text-xl leading-none text-white">{denominator}</span>
    </div>
  );
}
...
function ChoiceButton({
  ...
  return (
    <motion.button
      type="button"
      data-testid="fraction-balance-4-choice"
...
function CompletionCard({ onRestart }: { onRestart: () => void }) {
  return (
    <motion.section
      data-testid="fraction-balance-4-complete"
    ...
    >
      ...
      <h2 className="mt-4 text-3xl font-black text-white">Kesir terazisi dengede!</h2>
      <p className="mt-2 max-w-md text-sm font-bold leading-relaxed text-white/62 md:text-base">
        Kesir türü, denklik, karşılaştırma, bütün-parça ve eş paydalı işlem görevlerini tamamladın.
      </p>
      <div className="mt-5 w-full rounded-3xl border border-white/10 bg-black/24 p-4 text-left">
        ...
        <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
          {FRACTION_BALANCE_ATOMS.map((atom) => (
      ...
      <div className="mt-5 flex w-full flex-col gap-3 sm:flex-row">
        <motion.button
          type="button"
          data-testid="fraction-balance-4-restart"
