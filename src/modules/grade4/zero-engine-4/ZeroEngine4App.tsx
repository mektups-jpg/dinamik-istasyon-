...
import { createZeroEngineTasks, ZERO_ENGINE_ATOMS, type ZeroEngineTask } from './zeroEngine4Tasks';

const MODULE_ID = 'zero-engine-4';

export default function ZeroEngine4App() {
  const { unlockAtom, unlockModule } = useAtomStore();
  const { addScore } = useGameStore();
  const [tasks, setTasks] = useState<ZeroEngineTask[]>(() => createZeroEngineTasks());
  ...
  useEffect(() => {
    ...
    ZERO_ENGINE_ATOMS.forEach((atom) => unlockAtom(atom.id));
  ...
  const restart = () => {
    setTasks(createZeroEngineTasks());
    setTaskIndex(0);
    setSelected(null);
    setFeedback('idle');
    setIsComplete(false);
  ...
  const botMessage: BotMessage = useMemo(() => {
    if (isComplete) {
      return { id: 900, text: 'İşlem atölyesi tamam! Sıfır ekledin, sıfır sildin, tahmin ettin, basamakları korudun ve eşitliği dengeledin.', type: 'success' };
    }
    if (feedback === 'success') {
  ...
  return (
    <div className="relative h-full w-full overflow-x-hidden overflow-y-auto bg-[#070B12] pb-32 text-white">
      ...
      <header className="relative z-20 mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-7">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-xl font-black leading-tight text-[#2EE7FF] md:text-2xl">İşlem Kısayolları Atölyesi</h1>
            <p className="mt-1 font-mono text-[10px] font-black uppercase tracking-[0.18em] text-white/48 md:text-xs">
              İlkokul 4. Sınıf / Sıfır, Elde, Bozma ve Tahmin
            </p>
          </div>
        </div>
      ...
      <main className="relative z-10 mx-auto grid min-h-[calc(100vh-112px)] w-full max-w-7xl grid-cols-1 gap-5 px-4 py-3 md:px-7 lg:grid-cols-[minmax(0,1fr)_390px]">
        <section
          data-testid="zero-engine-4-stage"
        ...
        <aside
          data-testid="zero-engine-4-control-panel"
        ...
        >
          {isComplete ? (
            <div className="flex h-full min-h-[360px] flex-col items-center justify-center text-center">
              ...
              <h2 className="mt-5 text-2xl font-black">Atölye tamam!</h2>
              <p className="mt-2 max-w-xs text-sm font-bold leading-relaxed text-white/58">İşlem adımlarını ve eşitlik dengesini doğru seçtin.</p>
            </div>
          ) : (
            <>
              ...
              <div
                data-testid="zero-engine-4-feedback"
...
function OperationStage({ task, feedback }: { task: ZeroEngineTask; feedback: 'idle' | 'success' | 'error' }) {
  return (
...
function VisualParts({ task }: { task: ZeroEngineTask }) {
  if (task.kind === 'zero-multiply') {
    return (
      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
        {task.visualParts.map((part, index) => (
          <motion.div
            ...
            className="grid min-h-20 min-w-20 place-items-center rounded-3xl border text-4xl font-black"
            style={{ borderColor: `${task.accent}66`, backgroundColor: `${task.accent}17`, color: part === '0' ? task.accent : '#fff' }}
          >
            {part}
          </motion.div>
        ))}
  ...
  if (task.kind === 'column-add' || task.kind === 'column-sub') {
    return (
      <div className="mx-auto mt-5 max-w-md rounded-3xl border border-white/10 bg-black/28 p-5 font-mono text-3xl font-black">
        {task.visualParts.map((part, index) => (
          <p key={`${part}-${index}`} className={index === 2 ? 'mt-4 text-center text-sm font-black text-white/46' : 'text-right text-white'}>
            {part}
          </p>
        ))}
    ...
    <div className="mt-5 grid gap-3 md:grid-cols-3">
      {task.visualParts.map((part, index) => (
        <motion.div
          ...
          className="grid min-h-24 place-items-center rounded-3xl border bg-white/[0.06] p-4 text-center text-2xl font-black text-white"
          style={{ borderColor: `${task.accent}44` }}
        >
          {part}
        </motion.div>
      ))}
...
function ChoiceButton({
  ...
  return (
    <motion.button
      type="button"
      data-testid="zero-engine-4-choice"
...
function CompletionCard({ onRestart }: { onRestart: () => void }) {
  return (
    <motion.section
      data-testid="zero-engine-4-complete"
    ...
    >
      ...
      <h2 className="mt-4 text-3xl font-black text-white">İşlem atölyesi tamam!</h2>
      <p className="mt-2 max-w-md text-sm font-bold leading-relaxed text-white/62 md:text-base">
        Sıfır ekleme, sıfır silme, tahmin, elde, bozma ve eşitlik dengeleme adımlarını doğru seçtin.
      </p>
      ...
      <div className="mt-5 flex w-full flex-col gap-3 sm:flex-row">
        <motion.button
          type="button"
          data-testid="zero-engine-4-restart"
