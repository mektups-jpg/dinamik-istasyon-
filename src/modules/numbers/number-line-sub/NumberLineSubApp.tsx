...
import { ArrowLeft, ArrowRight, ArrowLeftCircle, CheckCircle2, Star, Sparkles, BatteryWarning } from 'lucide-react';
...
export default function NumberLineSubApp() {
  ...
  const startNum = 12;
  const jumpNum = 5;
  const targetNum = startNum - jumpNum; // 7
  ...
  useEffect(() => {
    if (phase === 1 && position === startNum && botState === 'idle') {
      ...
      setTimeout(() => {
        setPhase(3);
        setShowSuccess(true);
        unlockAtom("G1.NUM.001.2"); // Geriye ritmik sayma
        unlockAtom("G1.NUM.003.1"); // Çıkarma işlemi
        addScore(50);
      }, 600);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] text-white font-sans flex flex-col w-full h-full relative overflow-hidden">
      ...
      <header className="p-6 border-b border-gray-800/50 bg-[#0B0C10]/50 backdrop-blur-md flex items-center gap-4 relative z-30">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#FF3366] flex items-center gap-2">
            <BatteryWarning className="w-5 h-5" /> Sayı Doğrusunda Çıkarma
          </h1>
          <p className="text-sm text-gray-400">İlkokul 1. Sınıf • Çıkarma İşlemi</p>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-8 relative z-20">
        ...
        <div className="mt-32 flex gap-6 z-20">
          <motion.button 
            ...
            className="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-[#FF3366] to-[#E6004C] hover:from-[#FF668C] hover:to-[#FF1A66] text-white disabled:opacity-50 disabled:cursor-not-allowed rounded-2xl text-lg font-bold transition-all shadow-[0_0_20px_rgba(255,51,102,0.4)]"
          >
          ...
          {phase === 2 && (
            <motion.button 
              ...
              className="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-[#FF8800] to-[#E67300] hover:from-[#FFAA33] hover:to-[#FF8800] text-white disabled:opacity-50 disabled:cursor-not-allowed rounded-2xl text-lg font-bold transition-all shadow-[0_0_20px_rgba(255,136,0,0.4)] ml-4"
            >
        ...
        {showSuccess && (
          ...
          >
            ...
            >
              ...
              <p className="text-gray-400 mb-8 text-lg">Çıkarma işleminin geriye doğru saymak olduğunu kanıtladın.</p>
              ...
              <div className="bg-[#0B0C10]/80 rounded-2xl p-6 mb-8 text-left border border-gray-800">
                ...
                <div className="space-y-4">
                  <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white">G1.NUM.001.2</div>
                  ...
                  <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.7 }} className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-400">Çıkarma (Geriye Sayma)</div>
                    </div>
                  </motion.div>
                </div>
              </div>
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => navigate('/')}
                className="w-full py-5 bg-gradient-to-r from-[#FF3366] to-[#FF8800] text-white font-bold text-xl rounded-2xl shadow-[0_0_30px_rgba(255,51,102,0.3)] hover:shadow-[0_0_40px_rgba(255,51,102,0.5)] transition-all"
              >
                Laboratuvardan Çık
              </motion.button>
            </motion.div>
