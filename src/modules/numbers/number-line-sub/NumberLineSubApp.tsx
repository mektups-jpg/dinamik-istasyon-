...
import { ArrowLeft, ArrowRight, ArrowLeftCircle, CheckCircle2, RotateCcw, Star, Sparkles, BatteryWarning } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAtomStore } from '../../../store/useAtomStore';
import { useGameStore } from '../../../store/useGameStore';
import Bot, { BotState } from '../../../components/characters/Bot';
import { createNumberLineSubTask, NUMBER_LINE_SUB_MAX_NUMBER } from './numberLineSubTask';

export default function NumberLineSubApp() {
  const navigate = useNavigate();
  const { unlockAtom } = useAtomStore();
  const { addScore } = useGameStore();
  
  const [task, setTask] = useState(() => createNumberLineSubTask());
  const [position, setPosition] = useState(NUMBER_LINE_SUB_MAX_NUMBER);
  ...
  const maxNumber = NUMBER_LINE_SUB_MAX_NUMBER;
  ...
  useEffect(() => {
    if (phase === 1 && position === startNum && botState === 'idle') {
      ...
      setTimeout(() => {
        setPhase(3);
        setShowSuccess(true);
        unlockAtom("MAT.1.1.5.2");
        unlockAtom("MAT.1.2.1.2");
        addScore(50);
      }, 600);
    }
  ...
  const restartGame = () => {
    setTask((currentTask) => createNumberLineSubTask(currentTask));
    setPosition(NUMBER_LINE_SUB_MAX_NUMBER);
    setPhase(1);
  ...
  const handleJump = (dir: number) => {
    ...
    setTimeout(() => setBotState('idle'), 400);
  };
  ...
  return (
    <div className="h-full bg-[#0B0C10] text-white font-sans flex flex-col w-full relative overflow-x-hidden overflow-y-auto">
      ...
      {/* Glowing Orbs - Kırmızı/Turuncu tonları (Enerji azalması) */}
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#FF3366] rounded-full mix-blend-screen filter blur-[150px] opacity-20 animate-pulse pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-[#FF8800] rounded-full mix-blend-screen filter blur-[150px] opacity-10 animate-pulse pointer-events-none" />
      ...
      <header className="px-4 py-3 md:p-5 border-b border-gray-800/50 bg-[#0B0C10]/70 backdrop-blur-md flex items-center gap-3 md:gap-4 relative z-30">
        ...
        <div>
          <h1 className="text-lg md:text-xl font-bold text-[#FF3366] flex items-center gap-2 leading-tight">
            <BatteryWarning className="w-5 h-5 shrink-0" /> Sayı Doğrusunda Çıkarma
          </h1>
          <p className="text-xs md:text-sm text-gray-400">İlkokul 1. Sınıf • Çıkarma İşlemi</p>
        </div>
      </header>
      ...
      <main className="flex-1 min-h-0 flex flex-col items-center justify-center gap-3 px-4 py-3 md:px-8 md:py-5 relative z-20">
        ...
        >
          {/* Card inner glow */}
          <div className="absolute inset-0 bg-gradient-to-r from-[#FF3366]/5 to-[#FF8800]/5 pointer-events-none" />
          
          <h2 className="text-2xl md:text-3xl font-bold mb-2 relative z-10 leading-tight">
            {phase === 1 && <span className="text-white">Robotu <span className="text-[#FF3366] drop-shadow-[0_0_10px_rgba(255,51,102,0.8)]">{startNum}</span> noktasına götür.</span>}
            {phase === 2 && <span className="text-white">Enerji azalıyor! <span className="text-[#FF8800] drop-shadow-[0_0_10px_rgba(255,136,0,0.8)]">{jumpNum} adım</span> geriye zıpla!</span>}
          ...
          <p className="text-gray-400 text-sm md:text-base relative z-10 leading-snug">
            ...
            {phase === 2 && "Çıkarma işlemi geriye doğru saymaktır."}
        ...
        <div className="w-full max-w-5xl relative h-32 md:h-40 flex items-center">
          ...
          <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 h-3 bg-gray-800 rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-gray-700">
            {/* Progress fill */}
            <motion.div 
              className="absolute left-0 top-0 bottom-0 bg-gradient-to-r from-[#FF3366]/20 to-[#FF3366]/50 rounded-full"
                ...
                <div className="relative flex items-center justify-center mb-4">
                  <motion.div 
                    className={`w-6 h-6 rounded-full border-4 z-0 transition-colors duration-300 ${
                      isTarget 
                        ? 'border-[#FF3366] bg-[#FF3366]/20 shadow-[0_0_20px_#FF3366]' 
                        : isPassed 
                          ? 'border-[#FF3366]/50 bg-[#0B0C10]' 
                  ...
                  />
                  {/* Small inner dot */}
                  <div className={`absolute w-1.5 h-1.5 rounded-full ${isPassed ? 'bg-[#FF3366]' : 'bg-gray-600'}`} />
                </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 md:gap-5 z-20">
          ...
          >
            <span className="text-gray-200">Sağa 1 Zıpla</span>
          ...
          {phase === 2 && (
            <motion.button 
              ...
              className="min-h-12 flex items-center gap-2 md:gap-3 px-5 md:px-6 py-3 md:py-4 bg-gradient-to-r from-[#FF8800] to-[#E67300] hover:from-[#FFAA33] hover:to-[#FF8800] text-white disabled:opacity-50 disabled:cursor-not-allowed rounded-2xl text-base md:text-lg font-bold transition-all shadow-[0_0_20px_rgba(255,136,0,0.4)]"
            >
              <span className="drop-shadow-sm">Geriye Koş</span>
        ...
        {showSuccess && (
          ...
          >
            <motion.div 
              ...
              className="bg-gradient-to-b from-[#1F2833] to-[#121212] p-4 md:p-5 rounded-3xl md:rounded-[2.5rem] max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto w-full border border-gray-700 text-center shadow-[0_0_100px_rgba(255,51,102,0.2)] relative"
            >
              ...
              <p className="text-gray-400 mb-3 text-sm md:text-base leading-snug">Çıkarma işleminin geriye doğru saymak olduğunu kanıtladın.</p>
              ...
              <div className="bg-[#0B0C10]/80 rounded-2xl p-3 md:p-4 mb-3 text-left border border-gray-800">
                <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-[#FF3366]" /> Kazanılan Atomlar
                </h3>
                <div className="space-y-2">
                  <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white">MAT.1.1.5.2</div>
                      <div className="text-xs text-gray-400">Geriye Ritmik Sayma</div>
                    </div>
                  </motion.div>
                  <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.7 }} className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white">MAT.1.2.1.2</div>
                      <div className="text-xs text-gray-400">Miktarın Azalması</div>
                    </div>
                  </motion.div>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={restartGame}
                className="mb-2 flex w-full min-h-11 items-center justify-center gap-2 py-2.5 md:py-3 bg-gradient-to-r from-[#FF3366] to-[#FF8800] text-white font-bold text-lg rounded-2xl shadow-[0_0_30px_rgba(255,51,102,0.3)] hover:shadow-[0_0_40px_rgba(255,51,102,0.5)] transition-all"
              >
